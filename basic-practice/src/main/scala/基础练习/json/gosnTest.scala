...
object GosnTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val after1 = payload.getAsJsonObject.get("after").getAsJsonObject.addProperty("1", "2")


    val be: util.Set[Map.Entry[String, JsonElement]] = payload.getAsJsonObject.get("after").getAsJsonObject.entrySet()
    val fe: util.Set[String] = payload.getAsJsonObject.get("after").getAsJsonObject.keySet()
    val after3: JsonElement = payload.getAsJsonObject.get("source")
    println(be.iterator().next().getKey)
    println(after)
    println(fe.iterator().hasNext)

    val id: JsonElement = after.getAsJsonObject.get("id")
    println(id)
    val op: JsonElement = payload.getAsJsonObject.get("op")
    payload.getAsJsonObject.add("after", after3)
    println(op)
    println(payload)
  }
}
